<template>
	<div class="index">
		<div id="container">
			<img class="muyu" src="@/static/woodenfish/11.png" @click="play" />

			<!--手动的棍子-->
			<img class="gunzi" v-if="!data.auto" src="@/static/woodenfish/22.png" />
			<!--自动的棍子-->
			<img class="gunzi1" v-if="data.auto" src="@/static/woodenfish/22.png" />
			<!--木鱼声-->
			<audio id="music">
				<source src="@/static/woodenfish/1.mp3" />
			</audio>
			<!--1s延迟木鱼声-->
			<audio id="music1">
				<source src="@/static/woodenfish/2.mp3" />
			</audio>
			<div id="jia1"></div>
			<button class="bu" @click="autob">{{
        data.auto ? "手动" : "自动"
      }}</button>
		</div>
	</div>
		...
		setup() {
			...
			function play() {
				if (data.auto == 0) {
					...
					uni.setStorageSync("muyutime", store.state.gongde);
					...
					audio.play(); //播放音乐
					store.commit("gd", 1); //功德加1
					uni.setStorageSync("muyutime", store.state.gongde); //存到本地
					var jia = document.getElementById("jia1");
					var divv = document.createElement("div");
